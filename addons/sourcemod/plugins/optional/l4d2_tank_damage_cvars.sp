...
new Handle:vs_tank_vehicle_damage;
...
public OnPluginStart()
{
	...
	vs_tank_vehicle_damage = CreateConVar("vs_tank_vehicle_damage", "100", "n/a");
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damageType, &weapon, Float:damageForce[3], Float:damagePosition[3]) 
{
	...
	{
		PrintToChatAll("*DEBUG*: Attack to incapped client");
		if(GetEntProp(attacker, Prop_Send, "m_zombieClass") == 8) 
		{
			PrintToChatAll("*DEBUG*: Damage by hand");
			damage = GetConVarFloat(vs_tank_pound_damage);
		}
		else
		{
			PrintToChatAll("*DEBUG*: Damage by physical");
			damage = GetConVarFloat(vs_tank_vehicle_damage);
		}
	}
